...
class RequestsTorGetter(TorGetter):
    ...
    def fetch(self, url, port):
        ...
        def _stream_event(controller, event):
            # TODO: note that this is also where we will attach streams to custom circuits
            #       ... to do so, we will just check for StreamStatus.NEW and attach circuit
            #       to a free circuit from pool ...
            # TODO: convert this to TorStreamRouter
            nonlocal exit_fingerprint
            if event.status == StreamStatus.SUCCEEDED and event.circ_id:
                circ = controller.get_circuit(event.circ_id)
                exit_fingerprint = circ.path[-1][0]
                #print(f"PORT {port} --> StreamStatus: {str(event.status)} /// Exit Fingerprint: {exit_fingerprint}")
            else:
                #print(f"PORT {port} --> StreamStatus = {str(event.status)}")
                pass
